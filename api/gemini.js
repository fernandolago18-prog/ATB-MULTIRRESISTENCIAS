...
Devuelve SOLO el JSON, sin texto adicional, sin markdown, sin bloques de código.`;
  ...
  try {
    console.log("Attempting final stable connection to Gemini 1.5 Flash (v1beta)...");
    ...
    if (!response.ok) {
      ...
      console.error("Critical Google API Error:", JSON.stringify(err));
      
      const errorMessage = err.error?.message || `Error HTTP ${response.status}`;
      const errorCode = err.error?.status || 'UNKNOWN_ERROR';
      
      return res.status(response.status).json({ 
        error: `Error de Google AI (${errorCode}): ${errorMessage}`,
        details: err.error
      });
    }
    ...
    if (!data.candidates || data.candidates.length === 0) {
      const reason = data.promptFeedback?.blockReason || "Desconocido (posible filtro de seguridad)";
      return res.status(500).json({ error: `La IA no generó respuesta. Razón: ${reason}` });
    }
